...
def execute_select_query():
    ...
    select_query = f"SELECT * FROM {TARGET_SCHEMA}.{TARGET_TABLE}"
    print(select_query)
...
def handler(event, context: LambdaContext):
    try:
        result_records = execute_select_query()
        return {
            "Status": 200,
            "body": json.dumps({"Status": "success", "result_value": result_records, "ErrorReason": "None"}),
        }
